...
let URL = "https://teachablemachine.withgoogle.com/models/fQecfI1nY/";
...
switch(poseNumber) {
            case '1':
                if (time >= 5 && time <= 6.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 8.5 && time <= 9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if ((time >= 7 && time <= 8 && !poseState.firstWindowTriggered) ||
                    (time >= 11.5 && time <= 7.5 && !poseState.secondWindowTriggered)) {
                    if (time <= 13.0) {
                ...
                break;
            case '4':
                if (time >= 13.5 && time <= 14.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 15.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/fQecfI1nY/");
    }
}
